...
public class LegacyStoreManagerGateway {
  ...
  public void onStoreEvent(@jakarta.enterprise.event.Observes(during = jakarta.enterprise.event.TransactionPhase.AFTER_SUCCESS) StoreEvent event) {
    if (event.operation == StoreEvent.Operation.CREATE) {
      createStoreOnLegacySystem(event.store);
    } else if (event.operation == StoreEvent.Operation.UPDATE) {
